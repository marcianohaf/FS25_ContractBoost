...
function MissionBorrow:onSpawnedVehicle(superFunc, vehicle, ...)
    ...
    if vehicle ~= nil then
        ...
        if ContractBoost.debug then
            printf(MOD_NAME..':BORROW :: onSpawnedVehicle %s | isPallet %s', vehicle[1].configFileNameClean, MissionBorrow.pallets[configNameClean] and "yes" or "no")
        end
...
function MissionBorrow:addFillItemsToMissionTools()
    if ContractBoost.debug then print(MOD_NAME..':BORROW :: fillMissionTools') end
    ...
    if ContractBoost.debug then print(MOD_NAME..':BORROW :: fillMissionTools complete') end
end